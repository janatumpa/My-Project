...
if(isset($_POST['submit']))
{
    ...
    foreach ($arr as $id) {
       ...
       mysql_query("delete from pro_sent where M_Id='".$id."'");
    }
    $msg = "Deleted Successfully!";
    header("Location:sentmail.php?msg=$msg");
}
?>
<!DOCTYPE html>
              ...
              <div class="panel-body">
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tr>
                      ...
                      <th colspan="3"> 
                      <div class="mgr-20 menu-btn"><button type="submit" name="submit" role="button"><i class="icon-trash append-icon vd_green"></i> Delete</button></div> 
                      </th>
                    </tr>
                  </thead>
                   ...
                   $c=1;
                    $rows_per_page=12;
                         if(isset($_REQUEST['page']))
                          ...
                          $res_product=mysql_query("select * from ".TABLE_PREFIX."sent WHERE Admin_id = '".$_SESSION['admin_id']."'");
                          $totnumrow=mysql_num_rows($res_product);
                          $totrow=ceil($totnumrow/$rows_per_page);
                            
                            $subCatSql="select * from ".TABLE_PREFIX."sent WHERE Admin_id = '".$_SESSION['admin_id']."' limit $offset,$rows_per_page ";
                   ...
                   ?>
                    <tr>
                      <td style="width:20px"><div class="vd_checkbox">
                          <input name="chk_id[]" type="checkbox" id="<?php echo $c ?>" value="<?=$FetchRows['M_Id']?>" class="checkbox-group mail">
                          <label for="<?php echo $c ?>" ></label>
                        </div></td>  
<!--                      <td><?php echo $FetchRows['Email_Id']; ?></td>-->
                      <td><strong><?php echo $FetchRows['Subject']; ?></strong>-<?php echo $FetchRows['Message']; ?></td>
                      <td style="width:100px" class="text-right"><strong><?php echo date('d M Y',strtotime($FetchRows['Date'])); ?></strong></td>
                      <td class="menu-action rounded-btn">
                       <a href="view_sentmail.php?M_ID=<?php echo $FetchRows['M_Id']; ?>" class="btn menu-icon vd_bg-green" data-placement="top" data-toggle="tooltip" data-original-title="view">
                                           <i class="fa fa-eye"></i>
                        </a>                                   
