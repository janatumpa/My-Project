 ...
 ?>
   ...
   <div class="vd_content-wrapper">
    ...
    <div data-action="remove-navbar" data-original-title="Remove Navigation Bar Toggle" data-toggle="tooltip" data-placement="bottom" class="remove-navbar-button menu"> <i class="fa fa-arrows-h"></i> </div>
                    ...
                    <table class="table table-striped table-hover">
                      ...
                      <?php
                        $rows_per_page=12;
                         ...
                         // echo "SELECT * from pro_dev_registration where Role = 'vertical_head' ";
                          $vleave="SELECT D_id from pro_dev_registration where Role = 'vertical_head' ";
                          $res_product=mysql_query($vleave);
                          $totnumrow=mysql_num_rows($res_product);
                          $totrow=ceil($totnumrow/$rows_per_page);
                            
                            $subCatSql="SELECT * from pro_dev_registration  limit $offset,$rows_per_page ";
                          $ResSqlUser=mysql_query($subCatSql) or mysql_error();
                          
                     
                 $c = 1;
                 $SqlUser = "SELECT * FROM pro_dev_leaveapp where userid IN ($vleave)";
     ...
     <td>
      <select class="span9 chosen"  id="stat<?=$FetchRows['id']?>" onChange="changestatus(this.value,'<?=$FetchRows['id']?>')">
									 <option value="true" <?=$FetchRows['status'] == 'Yes' ? 'selected' : ''?>>Approve</option>
									 <option value="false" <?=$FetchRows['status'] == 'No' ? 'selected' : ''?>>Rejected</option>
                   <option value="" <?=$FetchRows['status'] == '' ? 'selected' : ''?>>Pending</option>
									 </select>
                  </td>
								</tr>
                <?php
...
<script>
	...
	{
		$.post('status/status_d_leave.php',{ stat : stat , id : id });
	}
</script>
