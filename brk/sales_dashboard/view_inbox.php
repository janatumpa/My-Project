...
if($_SESSION['userid'] == "" && $_SESSION['loggedIn'] == ""){
	...
	if(isset($_GET['M_ID']) && $_GET['M_ID'] != "")
    {
		...
		$m_record = mysql_query("select * from pro_inbox where M_Id='$Mail_id'");
    $rows = mysql_fetch_array($m_record);
    
        mysql_query("UPDATE pro_inbox SET status='r' WHERE M_Id='".$Mail_id."'");
    }

    if($rows['S_id'] != 0)
     {
         $fet = mysql_fetch_array(mysql_query("SELECT Sale_Name as name FROM pro_sale_registration WHERE S_id='".$rows['S_id']."'"));
     }

     if($rows['D_id'] != 0)
     {
         $fet = mysql_fetch_array(mysql_query("SELECT Dev_Name as name FROM pro_dev_registration WHERE D_id='".$rows['D_id']."'"));
     }
    if($rows['Admin_id'] != 0)
     {
         $fet = mysql_fetch_array(mysql_query("SELECT admin_name as name FROM pro_admin WHERE admin_id='".$rows['Admin_id']."'"));
     }
?>
                      ...
                      <div class="form-group  clearfix">
                        ...
                        <div class="col-sm-10 controls">
                         <?php echo $rows['Message']; ?><br/>
                            <?=date('d M Y H:i:s', strtotime($rows['Date']))?>
                        </div>
                      </div>
