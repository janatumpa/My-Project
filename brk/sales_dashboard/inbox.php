...
if($_SESSION['userid'] == "" && $_SESSION['loggedIn'] == ""){
 ...
 $sql = "SELECT dreg.Dev_Name FROM pro_dev_registration dreg,pro_inbox pi WHERE dreg.D_id=pi.D_id";
            ...
            <?php } ?>
                   ...
                   $c=1;
                    $rows_per_page=12;
                         if(isset($_REQUEST['page']))
                          ...
                          $res_product=mysql_query("select * from ".TABLE_PREFIX."inbox WHERE Email_Id = '".$_SESSION['loggedIn']."'");
                          $totnumrow=mysql_num_rows($res_product);
                          $totrow=ceil($totnumrow/$rows_per_page);
                            
                            $subCatSql="select * from ".TABLE_PREFIX."inbox WHERE Email_Id = '".$_SESSION['loggedIn']."' ORDER BY M_Id DESC limit $offset,$rows_per_page ";
                ...
                //  $ResSqlUser = mysql_query($SqlUser);
                 while($FetchRows = mysql_fetch_array($ResSqlUser))
                 {
                     if($FetchRows['S_id'] != 0)
                     {
                         $fet = mysql_fetch_array(mysql_query("SELECT Sale_Name as name FROM pro_sale_registration WHERE S_id='".$FetchRows['S_id']."'"));
                     }
                    
                     if($FetchRows['D_id'] != 0)
                     {
                         $fet = mysql_fetch_array(mysql_query("SELECT Dev_Name as name FROM pro_dev_registration WHERE D_id='".$FetchRows['D_id']."'"));
                     }
                     
                     if($FetchRows['Admin_id'] != 0)
                     {
                         $fet = mysql_fetch_array(mysql_query("SELECT admin_name as name FROM pro_admin WHERE admin_id='".$FetchRows['Admin_id']."'"));
                     }
                   ?>
                    <tr>
                      <td style="width:20px"><div class="vd_checkbox">
                          <input id="<?php echo $FetchRows['M_Id']; ?>" type="checkbox" name="chk_id[]" value="<?php echo $FetchRows['M_Id']; ?>" class="checkbox-group mail">
                          <label for="<?php echo $FetchRows['M_Id']; ?>" ></label>
                        </div></td> 
                       <?php if($FetchRows['status'] == 'ur'){?>  
                       
                        <td> <a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>"><strong><?php echo $fet['name']; ?></strong></a></td>
                      
                        <td> <a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>"><strong><?php echo $FetchRows['Subject']; ?></strong>-<?php echo substr($FetchRows['Message'],0,75); ?></a></td>
                        <td style="width:170px" class="text-right"> <a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>"><strong><?php echo date('d M Y H:i A',strtotime($FetchRows['Date'])); ?></strong></a></td>
                      
                    <?php }else{?>
                        
                     <td><a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>"><?php echo $fet['name']; ?></a></td>
                      
                     <td><a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>"><?php echo $FetchRows['Subject']; ?>-<?php echo substr($FetchRows['Message'],0,75); ?></a></td>
                     <td style="width:170px" class="text-right"><a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>"><?php echo date('d M Y H:i A',strtotime($FetchRows['Date'])); ?></a></td>
                      
                    <?php }?>
<!--
                     <td class="menu-action rounded-btn" width="10%">
                       <a href="view_inbox.php?M_ID=<?php echo $FetchRows['M_Id']; ?>" class="btn menu-icon vd_bg-green" data-placement="top" data-toggle="tooltip" data-original-title="view">
                                           <i class="fa fa-eye"></i>
                        </a>                                   
                    <a href="delete/delinboxmail.php?id=<?php echo $FetchRows['M_Id']; ?>" onClick="javascript:return confirm('Are You Sure?');" class="btn menu-icon vd_bg-red" data-placement="top" data-toggle="tooltip" data-original-title="delete">
